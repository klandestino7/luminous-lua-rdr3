...
AddEventHandler('VP:BANKING:withdraw', function(amount)
...
AddEventHandler('VP:BANKING:deposit', function(amount)
    local _source = source
    local base = 0
    local _amount = tonumber(amount*100)
    local User = API.getUserFromSource(_source)      
...
AddEventHandler('VP:BANKING:balance', function(source)
    local _source = source
    local User = API.getUserFromSource(_source)      
    local Character = User:getCharacter()
    local call = json.decode(Character:getData(Character:getId(), 'charTable', 'banco'))  
...
AddEventHandler('VP:BANKING:balance2', function()
    local _source = source
    local User = API.getUserFromSource(_source)      
    local Character = User:getCharacter()
    local call = json.decode(Character:getData(Character:getId(), 'charTable', 'banco'))
...
AddEventHandler('VP:BANKING:sendmoney', function(source, id, amount)
    local _source = source
    local User = API.getUserFromSource(_source)      
    local Character = User:getCharacter()
...
AddEventHandler('VP:BANKING:recmoney', function(source, amount, name)
    local _name = name
    local _source = source
    local User = API.getUserFromSource(_source)      
    local Character = User:getCharacter()

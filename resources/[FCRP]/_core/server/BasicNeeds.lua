function API.varyHunger(variation)
    ...
    local hunger = json.decode(Character:getData('charTable', 'hunger'))
...
function API.varyThirst(variation)
    ...
    local thirst = json.decode(Character:getData('charTable', 'thirst'))
    ...
    if thirst then
        ...
        Character:setData('charTable', 'thirst', tonumber(math.floor(thirst * 100) / 100))

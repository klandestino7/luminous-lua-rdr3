...
Citizen.CreateThread(function()
	...
	if spawned >= 1 then
		Wait(8000)
		print('POLICECHECK' .. spawned)
		TriggerServerEvent('FRP:SHERIFF:checkjob')
	end
...
RegisterNetEvent('FRP:SHERIFF:PoliceCheck')
AddEventHandler('FRP:SHERIFF:PoliceCheck', function(police)
	PoliceCheck = police	
end)

RegisterNetEvent('FRP:SHERIFF:SheriffCheck')
AddEventHandler('FRP:SHERIFF:SheriffCheck', function(sheriff)
  ...
  while true do		
	...
	elseif WarMenu.IsMenuOpened('Armored') then 
		if WarMenu.Button('Pistola Mauser') then
			TriggerEvent('FRP:SHERIFF:giveweapon', 'weapon_pistol_semiauto')
		elseif WarMenu.Button('Bolt action rifle') then
			TriggerEvent('FRP:SHERIFF:giveweapon', 'WEAPON_RIFLE_BOLTACTION')
		elseif WarMenu.Button('Pump shotgun') then
			TriggerEvent('FRP:SHERIFF:giveweapon', 'WEAPON_SHOTGUN_PUMP')
		end
			WarMenu.Display()
  ...
  while true do		
	...
	local coords = GetEntityCoords(playerPed)
	  if WarMenu.IsMenuOpened('OfficerMenu') then
		if WarMenu.MenuButton('Interação Cidadão', 'citizen') then 	end	
		if WarMenu.MenuButton('Interação Veiculo', 'vehiclec') then 	end
		if PoliceCheck then
...
AddEventHandler('FRP:SHERIFF:giveweapon', function(weapon)
    Citizen.CreateThread(function()
...
RegisterNetEvent('FRP:SHERIFF:unrestrain')
AddEventHandler('FRP:SHERIFF:unrestrain', function()
	if Handcuffed then
		local playerPed = PlayerPedId()		
		Handcuffed = false
...
AddEventHandler('FRP:SHERIFF:putinvehicle', function()	
...
AddEventHandler('FRP:SHERIFF:vehiclep', function()
...
AddEventHandler('FRP:SHERIFF:outvehicle', function()	
...
AddEventHandler('FRP:SHERIFF:vehicleout', function()
	local playerPed = PlayerPedId()
...
AddEventHandler('FRP:SHERIFF:cuff', function()
    ped = GetPlayerPed(-1)
	ped2 = PlayerPedId()
...
AddEventHandler('FRP:SHERIFF:uncuff', function()
    ped = GetPlayerPed(-1)
    ped2 = PlayerPedId()  
	ClearPedTasks(ped)
...
AddEventHandler('FRP:SHERIFF:cuffcheck', function()
  local player, distance = GetClosestPlayer()
  if distance ~= -1 and distance <= 3.0 then   
...
AddEventHandler('FRP:SHERIFF:nyckelcheck', function()    
...
AddEventHandler('FRP:SHERIFF:unlockingcuffs', function()
  local player, distance = GetClosestPlayer()
...
-- RegisterNetEvent('FRP:SHERIFF:result')
-- AddEventHandler('FRP:SHERIFF:result', function(value)    
-- 	print(value)
-- end)

...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("nc_clothes:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("nc_clothes:insert_character_face", data)
            TriggerServerEvent("nc_clothes:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
Citizen.CreateThread(function()
	while true do
        ...
        if nearcloth < 5.0 then
            menu = {"clothesmenu", "Aperte ~g~M~s~ para trocar de Roupas $"..StoreCost}
        elseif neartat < 5.0 then
            menu = {"tattoomenu", "Press ~g~M~s~ para fazer Tatuagem $"..StoreCost}
        elseif nearbarber < 5.0 then
            menu = {"barbermenu", "Press ~g~M~s~ para mudar o Cabelo $"..StoreCost}
...
RegisterNetEvent("nc_clothes:inService")
AddEventHandler("nc_clothes:inService", function(service)
    isService = service
end)


RegisterNetEvent("nc_clothes:hasEnough")
AddEventHandler("nc_clothes:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("nc_clothes:retrieve_tats")
...
RegisterNetEvent("nc_clothes:setclothes")
AddEventHandler("nc_clothes:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    ...
    Citizen.Wait(500)
	TriggerEvent("facewear:update")
    TriggerServerEvent("nc_clothes:get_character_face")
    TriggerServerEvent("nc_clothes:retrieve_tats")
	TriggerServerEvent("Police:getMeta")
end)


RegisterNetEvent("nc_clothes:defaultReset")
AddEventHandler("nc_clothes:defaultReset", function()
   -- local LocalPlayer = exports["np-base"]:getModule("LocalPlayer")
    local gender = LocalPlayer:getCurrentCharacter().gender
    Citizen.Wait(1000)
    if gender ~= 0 then
...
RegisterNetEvent("nc_clothes:settattoos")
AddEventHandler("nc_clothes:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("nc_clothes:setpedfeatures")
AddEventHandler("nc_clothes:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterNetEvent('nc_clothes:outfits')
AddEventHandler('nc_clothes:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("nc_clothes:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("nc_clothes:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("nc_clothes:get_outfit", pId)
    else
        TriggerServerEvent("nc_clothes:list_outfits")
    end
end)

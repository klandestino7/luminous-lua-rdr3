...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(10)
        local fishingrod = GetHashKey("WEAPON_FISHINGROD")
        local bait = GetHashKey("p_baitWorm01x")
        if DoesEntityExist(ply) then
            if HasPedGotWeapon(ply , fishingrod, 0, 0) and IsControlJustPressed(0, 0xE30CD707) then --[R]
                TaskSwapFishingBait(ply, "p_baitWorm01x", 1)
            end
        end
    end
end)

                ...
                elseif WarMenu.IsMenuOpened('dollar') then 
                    for y,e in pairs(Config.Shops[k].items) do
                        if WarMenu.Button(Config.Shops[k].items[y].itemName..'~pa~ '..Config.Shops[k].items[y].money..' $') then
                            TriggerServerEvent('fcrp_shops:buyedItem', false, Config.Shops[k].items[y])
                        end
                    end
                    WarMenu.Display()
                elseif WarMenu.IsMenuOpened('gold') then 
                    for y,e in pairs(Config.Shops[k].items) do
                        if WarMenu.Button(Config.Shops[k].items[y].itemName..'~pa~ '..Config.Shops[k].items[y].gold..' *') then
                            TriggerServerEvent('fcrp_shops:buyedItem', true, Config.Shops[k].items[y])
                        end
                    end
                    WarMenu.Display()
                end
...
RegisterNetEvent('fcrp_shops:message')
AddEventHandler('fcrp_shops:message', function(message)
    DisplayHelp(message)
end)

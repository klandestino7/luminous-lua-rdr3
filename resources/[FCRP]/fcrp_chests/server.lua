...
AddEventHandler(
    ...
    function(capacity, x, y, z, h)
        ...
        local rowsWithId = dbAPI.query('FCRP/CreateChest', {charid = Character:getId(), position = json.encode(position), type = 1, capacity = capacity})
...
AddEventHandler(
    ...
    function(chestId)
        ...
        -- User:viewInventoryAsPrimary(chestInventory)
    end
)
...
function getItemIdFromCapacity(capacity)
    if capacity == 25 then
        return 'kit_chest_small'
    end

    if capacity == 50 then
        return 'kit_chest_medium'
    end

    if capacity == 100 then
        return 'kit_chest_large'
    end

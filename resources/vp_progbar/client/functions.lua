...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or vp_action.useWhileDead then
        if not isDoingAction then
            ...
            exports['vp_notify']:SendAlert('error', 'Already Doing An Action', 1000)
        end
    else
        exports['vp_notify']:SendAlert('error', 'Cannot Perform An Action While Dead', 1000)
    end
end

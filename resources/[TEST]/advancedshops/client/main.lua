...
RegisterNUICallback('refresh', function(data, cb)
	...
	--	ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getOwnedShop', function(data)
	--		ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getShopItems', function(result)
...
RegisterNUICallback('buy', function(data, cb)	
	TriggerServerEvent('FRP:ADVANCEDSHOPs:Buy', number, data.Item, data.Count)
	Cart = {}
end)
...
AddEventHandler('FRP:ADVANCEDSHOP:hasEnteredMarker', function(zone)
	if zone == 'center' then
		ShopId = zone
		number = zone
		Msg  = 'Press ~INPUT_CONTEXT~ to open the shopcenter.'
	...
	elseif zone >= 100 then
		ShopId = zone
		number = zone
		Msg  = 'Press ~INPUT_CONTEXT~ to ~r~rob ~w~the shop.'
	end
end)

AddEventHandler('FRP:ADVANCEDSHOP:hasExitedMarker', function(zone)
	ShopId = nil
end)
...
-- 			ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getShopList', function(data)
...
-- 					TriggerServerEvent('FRP:ADVANCEDSHOPs:BuyShop', name, data.current.price, data.current.shop, data.current.bought)
-- 					menu2.close()
...
-- 			TriggerServerEvent('FRP:ADVANCEDSHOPs:takeOutMoney', amount, number)
...
-- 			TriggerServerEvent('FRP:ADVANCEDSHOPs:addMoney', amount, number)
-- 			menu3.close()
-- 				end,
-- 				function(data3, menu3)
-- 			menu3.close()
...
--               TriggerServerEvent('FRP:ADVANCEDSHOPs:SellShop', number)
--               menu4.close()
-- 			end
-- 		    	end,
-- 		    	function(data4, menu4)
...
--             TriggerServerEvent('FRP:ADVANCEDSHOPs:changeName', number, data5.value)
--             menu5.close()
--                		end,
--                 	function(data5, menu5)
--                 	menu5.close()
...
-- 	ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getTime', function(time)
-- 	ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getAllShipments', function(items)
...
-- 			TriggerServerEvent('FRP:ADVANCEDSHOPs:GetAllItems', id)
-- 		end
...
-- 				}, function(data2, menu2)
-- 					menu2.close()
-- 					TriggerServerEvent('FRP:ADVANCEDSHOP:MakeShipment', id, data.current.value, data.current.price, tonumber(data2.value), data.current.labels)
...
-- 		menu2.close()
--     	TriggerServerEvent('FRP:ADVANCEDSHOPs:RemoveItemFromShop', number, count, name)
...
--   ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getInventory', function(result)
...
-- 				menu3.close()
-- 				TriggerServerEvent('FRP:ADVANCEDSHOPs:setToSell', number, itemName, count, price)
...
Citizen.CreateThread(function ()
  while true do
	...
	if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
	  ...
	  TriggerEvent('FRP:ADVANCEDSHOP:hasEnteredMarker', currentZone)
	end

	if not isInMarker and HasAlreadyEnteredMarker then
	  HasAlreadyEnteredMarker = false
	  TriggerEvent('FRP:ADVANCEDSHOP:hasExitedMarker', LastZone)
	end
  end
end)

RegisterNetEvent('FRP:ADVANCEDSHOPs:setBlip')
AddEventHandler('FRP:ADVANCEDSHOPs:setBlip', function()

  	ESX.TriggerServerCallback('FRP:ADVANCEDSHOP:getOwnedBlips', function(blips)
...
RegisterNetEvent('FRP:ADVANCEDSHOPs:removeBlip')
AddEventHandler('FRP:ADVANCEDSHOPs:removeBlip', function()
...
function Robbery(id)
    ...
    -- TriggerServerEvent('FRP:ADVANCEDSHOPs:UpdateCurrentShop', id)

        ESX.TriggerServerCallback('FRP:ADVANCEDSHOP-robbery:getUpdates', function(result)
		ESX.TriggerServerCallback('FRP:ADVANCEDSHOP-robbery:getOnlinePolices', function(results)

			if result.cb ~= nil then
				if results >= Config.RequiredPolices then
                TriggerServerEvent('FRP:ADVANCEDSHOPs-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('FRP:ADVANCEDSHOPs-robbery:NotifyOwner', "~r~Your store ~b~(" .. result.name .. ')~r~ is under robbery', id)
	...
	while true do
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				...
				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('FRP:ADVANCEDSHOPs-robbery:GetReward', Id)
					TriggerServerEvent("FRP:ADVANCEDSHOPs-robbery:NotifyOwner", '~r~The robbery on your shop ~b~(' .. Name ..')~r~ was unfortunately successful!', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('FRP:ADVANCEDSHOPs-robbery:NotifyOwner', "~g~The robbery on your shop ~b~(" .. Name .. ')~g~ was not successful!', Id)

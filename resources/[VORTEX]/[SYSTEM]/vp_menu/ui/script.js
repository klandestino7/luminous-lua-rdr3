$(document).ready(function(){
  ...
  $('.openCoffre').on('click', function(e){
    e.preventDefault();
    $.post('http://vp_menu/togglecoffre', JSON.stringify({
      id: idEnt
    }));
    $(this).find('#texto').text($(this).find('#texto').text() == ' ❯ Abrir mala' ? ' ❯ Fechar mala' : ' ❯ Abrir mala');
  });

  $('.openCapot').on('click', function(e){
    e.preventDefault();
    $.post('http://vp_menu/togglecapot', JSON.stringify({
      id: idEnt
    }));
    $(this).find('#texto').text($(this).find('#texto').text() == ' ❯ Abrir capô' ? ' ❯ Fechar capô' : ' ❯ Abrir capô');
  });

  $('.lock').on('click', function(e){
    e.preventDefault();
    $.post('http://vp_menu/togglelock', JSON.stringify({
      id: idEnt
    }));
    $(this).find('#texto').text($(this).find('#texto').text() == ' ❯ Trancar' ? ' ❯ Destrancar' : ' ❯ Trancar');
  });
  ...
  $('.cheer').on('click', function(e){
    e.preventDefault();
    $.post('http://vp_menu/cheer', JSON.stringify({
      id: idEnt
    }));
  });
  ...
  $('.crosshair').on('click', function(e){
    ...
    $(".menu").removeClass('fadeIn');
    $.post('http://vp_menu/disablenuifocus', JSON.stringify({
      nuifocus: false
    }));
  });

...
$(".button-right").on('click', function() {
    ...
    if (facefeature == undefined) {
        $.post('http://vp_clothes/' + component, JSON.stringify({ id }));
    } else {
        $.post('http://vp_clothes/' + component, JSON.stringify({ facefeature: facefeature, id: id }));
    }
...
$(".button-left").on('click', function() {
    ...
    if (facefeature == undefined) {
        ...
        $.post('http://vp_clothes/' + component, JSON.stringify({ facefeature: facefeature, id }));
    }

});
function confirm(){
    $.post('http://vp_clothes/Confirm')
    $("#creatormenu").fadeOut(500);
}

function changeoptions(name) {
    var change = document.getElementById(name).value;
    $.post('http:/vp_clothes/' + name,  JSON.stringify({ change }));
}
...
window.addEventListener("keydown", throttle(function (ev) {
    ...
    if (_key) {
        $.post('http://vp_clothes/rotate', JSON.stringify({key: _key}))
    }
}, 50))

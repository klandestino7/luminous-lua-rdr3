...
function changeoptions(name) {
    ...
    $.post('http:/vp_charcreation/' + name, change);
}
                ...
                if (!selects[0].children[1].children[selected].className.includes("disabled")) {
                    if (!selects[0].children[1].children[selected].getAttribute("arrows") == true) {
                        ...
                        if (link == "createName") {
                            ...
                            $.post("http://vp_charcreation/nm", JSON.stringify({}));
                            console.log('Post NameClicked2');
                        } else if (link == "confirm") {
                            $.post("http://vp_charcreation/Confirm", JSON.stringify({}));
                            closeCreator();
                        } else {
                            $('.block.active').fadeOut(200, function() {
                                $('.block').removeClass('active');
...
$('.arrowvetement-right').on('click', function(e) {
    ...
    if (li != null) {
        if (id <= parseInt(max)) {
            var link = selects[0].children[1].children[selected].getAttribute("data-link");
            if (link != null) {
                $.post("http://vp_charcreation/" + link, JSON.stringify({
                    id
                }));
            }
...
function closeCreator() {
    $(".skinCreator").fadeOut(500);
    $.post("http://vp_charcreation/CloseCreator", JSON.stringify({}));
}

function allManagement(type) {
    $.post('http://vp_charcreation/CameraManagement', JSON.stringify({
        type: type,
    }));
    console.log(type);
}
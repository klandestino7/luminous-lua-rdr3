...
$(".button-right").on('click', function() {
    ...
    if (facefeature == undefined) {
        $.post('http://vp_creator/' + component, JSON.stringify({ id }));
    } else {
        $.post('http://vp_creator/' + component, JSON.stringify({ facefeature: facefeature, id: id }));
    }
...
$(".button-left").on('click', function() {
    ...
    if (facefeature == undefined) {
        ...
        $.post('http://vp_creator/' + component, JSON.stringify({ facefeature: facefeature, id }));
    }

});
function confirm(){
    $.post('http://vp_creator/CloseCreator')
    $("#creatormenu").fadeOut(500);
}
...
window.addEventListener("keydown", throttle(function (ev) {
    ...
    if (_key) {
        $.post('http://vp_creator/rotate', JSON.stringify({key: _key}))
    }
}, 50))

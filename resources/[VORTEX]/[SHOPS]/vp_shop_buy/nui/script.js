...
function buyItem() {
    if ($('.selected').length > 0) {
        ...
        $.post('http://vp_shop_buy/buyItem', JSON.stringify({ shopId: shopId, itemId: itemId, withGold: buyingWithGold }));
        buyingWithGold = false;
...
function select(element) {
    if ($(element).attr('id') != undefined) {
        ...
        $('.description-title').text('Comprar | ' + item_name);
...
function close() {
    $(".container").hide();
    $.post("http://vp_shop_buy/NUIFocusOff", JSON.stringify({}));
}

$(document).keyup(function(e) {
    if (e.key === "Escape") {
        $(".container").fadeOut(100);
        $.post('http://vp_shop_buy/focusOff');
    }
});
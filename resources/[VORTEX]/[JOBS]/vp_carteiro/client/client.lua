...
Citizen.CreateThread(
    function()
    for i = 0, #WorkStations, 0 do
        if pedsSpawned[i] == nil then
            local model = GetHashKey(WorkStations[i].ped)
            RequestModel(model)

            while HasModelLoaded(model) and rModel do
                Citizen.Wait(1000)
                rModel = RequestModel(model)               
            end
...
Citizen.CreateThread(
    ...
    if timer == 0 then
        ...
        if steps == 0 then
            for i = 0, #pedsSpawned, 0 do
                local posPed = GetEntityCoords(pedsSpawned[i])
                if GetDistanceBetweenCoords(posPed[0], posPed[1], posPed[2], posPlayer[0], posPlayer[1], posPlayer[2]) < 2.5 then
...
Citizen.CreateThread(
    function()
    if timing and timer ~= 0 then
        timing = Citizen.Wait(1000)
        timer = timer - 1
        timing = nil
    end
end)
...
function setSatchel(model)
    if HasModelLoaded(parseInt(model)) then
...
function getPosts()
    for i = 0, #WorkStations[postUsing].Posts, 0 do
